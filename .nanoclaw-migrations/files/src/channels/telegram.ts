...
export class TelegramChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    this.bot.on('message:photo', (ctx) => {
      ...
      const filename = `photo_${ctx.message.message_id}`;
      const timestamp = new Date(ctx.message.date * 1000).toISOString();
      const senderName =
        ctx.from?.first_name ||
        ctx.from?.username ||
      ...
      this.downloadFile(fileId, group.folder, filename).then(
        async (filePath) => {
          ...
          if (filePath) {
            // Resolve host path for image processing
            const hostPath = path.join(
              resolveGroupFolderPath(group.folder),
              'attachments',
              path.basename(filePath),
            );
            const processed = await processImage(hostPath);
            if (processed) {
              images = [processed];
            }
          }
    ...
    this.bot.on('message:voice', (ctx) => {
      ...
      this.downloadFile(fileId, group.folder, filename).then(
        async (containerPath) => {
          ...
          // Resolve host path for transcription
          const hostPath = path.join(
            resolveGroupFolderPath(group.folder),
            'attachments',
